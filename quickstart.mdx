...
**Step 4**: Download the latest version of Subtrace, set the `SUBTRACE_TOKEN` environment variable to the token you generated in the previous step, and use `subtrace run` to start your app.

```bash
curl -fsSL -o /usr/bin/subtrace https://subtrace.dev/download/latest/linux/amd64/subtrace && chmod +x /usr/bin/subtrace
